...
**Q：**為什麼本專案要有SQL資料庫呢 ? 
**A:**這個專案是要自己建立一個部落格後台系統，類似商業網誌如無名小站(已倒站)及痞客邦，或類似部落格架站軟體Wordpress的一種Blog系統，不過本專案不會有那麼多功能，只是想建立一個小型的Blog架站機制，練習動態網頁語法  

如果只透過HTML或網頁設計丙級所設計出來的網站，也許只用到了HTML跟CSS跟少量的繪圖軟體(PhotoImpact)，但是HTML只能建立簡易表單，**按下"送出"後，文件卻無法送出**，因為**資料沒有儲存的地方，所以按了也無法送出資料**  

因此想建立簡易表單，，官方能查到你輸入的資訊，那一定得透過程式碼儲存，在本專案中**你在HTML表單輸入資料後，會透過PHP傳到SQL後端伺服器**，部分網站可能會用到ASP.NET，可達到同樣的效果  
